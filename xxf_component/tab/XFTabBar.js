...
export class XFTabBar extends Component<Props, State> {
    static defaultProps = {
        tabMargin: 20,
        tabBarTextStyle: {},
        tabStyles: {
    ...
    calculateInterpolations = () => {
        ...
        for (let i = 0, len = inputRange.length; i < len; i += 1) {
            ...
            if (this.props.underlineWidth && this.props.underlineWidth > 0) {
                outputRangeLeft.push(this.tabState[key].x + ((this.tabState[key].width - this.props.underlineWidth) / 2));
                outputRangeWidth.push(this.props.underlineWidth);
            } else {
